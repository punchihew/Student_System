...
// Helper function for counting up to the target number
const useCountUp = (start: number, end: number, duration: number) => {
  const [count, setCount] = useState(start);

  useEffect(() => {
    let startTimestamp: number | null = null; // Correct type annotation
...
const AdminHome = () => {
  // Counting up numbers for each stat
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-10 bg-white/90 backdrop-blur-lg rounded-3xl p-8 max-w-5xl mx-auto shadow-2xl"
      >
        <motion.h1
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3, duration: 0.5 }}
          className="text-5xl font-extrabold text-blue-600 text-center mb-10"
        >
          Welcome, Admin!
        </motion.h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <motion.div
            ...
            className="bg-white p-8 rounded-lg shadow-xl text-center hover:scale-105 transition-all"
          >
            <motion.h2 className="text-4xl font-semibold text-blue-600">
              {totalStudents}
            </motion.h2>
